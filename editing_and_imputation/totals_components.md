...
### 6.1 Error Detection
If a total for the target period does not equal the sum of its corresponding components in the target period, then the method checks if the absolute difference between the sum of components for the target period and the total for the predictive period is less than or equal to a set threshold.  
...
However, if the absolute difference is greater than the threshold, then no correction is applied, and the responder is recontacted for confirmation, if the target variable is a returned value. 
...
| Contributor | Amend Total | Total Variable | Component Variable | Predictive Variable | Auxiliary Variable | Threshold | Correction Applied | Corrected Variable |
|---|---|---|---|---|---|---|---|---|
| 1 | 1 | 50 | 14, 20, 17, 13 | 53 | N/A | 10 | N | N/A |
| 2 | 0 | 175 | 78, 64, 17, 8 | 187 | N/A | 20 | Y | 82, 67, 18, 8 |
| 3 | 1 | 11 | 10, 5, 4, 0 |   | 15 | 5 | Y | 19 |
| 4 | 1 | 30 | 12, 8, 14, 9 | 38 | N/A | 99999 | Y | 43 |

Contributor 1: The absolute difference between the sum of the components and the predictive variable is greater than the threshold. Therefore, no automatic correction is applied, and the contributor is recontacted for confirmation. 

Contributor 2: The absolute difference between the sum of the components and the predictive value is equal to the threshold. Therefore, the correction is applied to the components, as the Amend Total variable = 0. The proportions of the components are corrected to equal the total.  

Contributor 3: The predictive value is missing but the user has populated the optional auxiliary variable. The absolute difference between the sum of the components and the auxiliary variable is less than the threshold. Therefore, the total was corrected to equal the sum of components, as Amend Total = 1. 

Contributor 4: The absolute difference between the sum of the components and the predictive value is less than the threshold which has been set to the maximum value. Therefore, the total was corrected to equal the sum of components, as Amend Total = 1. 
